...
for i = 1:20
    ...
    ey = [ey;0.00*sin(i)];
    ez = [ez;0.00*cos(i)];
    roll = [roll;0.0]; %Rotation about x-axis in radians
    pitch = [pitch;0.0]; %Rotation about y-axis in radians
    yaw = [yaw;0.0]; %Rotation about z-axis in radians
    
    exdot = [exdot;0];
    eydot = [eydot;0.0*cos(i)];
    ezdot = [ezdot;-0.0*sin(i)];

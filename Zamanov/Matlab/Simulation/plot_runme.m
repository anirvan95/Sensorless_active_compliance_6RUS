...
for i=1:length(ex)
	...
	[theta_one, theta_two, theta_three, theta_14, theta_15, theta_16, M_matrix, K_matrix, d_end_effector_wrt_vertex1rot] = inverse_kinematics(Top_matrix, Base_matrix, l1, L2, ex(i), ey(i), ez(i), r_p, top_matrix_regen);
	[Top_matrix_test, end_effector_test, Transformation] = DH_parameters(l1,L2,theta_one,theta_two,theta_three,theta_14,theta_15, theta_16, end_effector_regen, top_matrix_regen);
...
% 	
	for k = 1:5
        ...
        %plot3(top_matrix_regen(1,k),top_matrix_regen(2,k),top_matrix_regen(3,k),'*k');
        %hold on;
	end
...
% 	%%lets check fucking det of jacobian
% 	for j = 1:6
% 		if j ==1
% 			k = 1;
% 		elseif j ==2
